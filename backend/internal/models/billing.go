...
type Billing struct {
	...
	InvoiceNumber   string         `gorm:"unique;not null;size:50;index" json:"invoice_number" validate:"required"`
	...
	PaidAmount      float64        `gorm:"not null;type:decimal(12,2);default:0" json:"paid_amount"`
	DiscountAmount  *float64       `gorm:"type:decimal(12,2)" json:"discount_amount,omitempty"`
	PaymentStatus   string         `gorm:"type:enum('unpaid','partial','paid','cancelled');not null;default:'unpaid';index" json:"payment_status"`
	PaymentMethod   string         `gorm:"type:enum('cash','debit_card','credit_card','insurance','transfer','other')" json:"payment_method,omitempty"`
