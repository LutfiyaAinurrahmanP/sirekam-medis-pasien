...
type Billing struct {
	...
	PaidAmount      float64        `gorm:"not null;type:decimal(12,2);default:0" json:"paid_amount" validate:"required"`
	DiscountAmount  *float64       `gorm:"type:decimal(12,2)" json:"discount_amount,omitempty" validate:"omitempty"`
	PaymentStatus   string         `gorm:"type:enum('unpaid','partial','paid','cancelled');not null;default:'unpaid';index" json:"payment_status" validate:"required"`
	PaymentMethod   string         `gorm:"type:enum('cash','debit_card','credit_card','insurance','transfer','other')" json:"payment_method,omitempty" validate:"omitempty"`
