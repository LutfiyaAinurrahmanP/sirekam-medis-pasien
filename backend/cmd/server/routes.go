...
type RouteConfig struct {
	...
	TestTypeHandler    handlers.TestTypeHandler
...
func SetupRoutes(app *fiber.App, config *RouteConfig) {
	...
	admin.Use(middlewares.RequireAdmin())                                        // Require admin role
	{
		...
		department := admin.Group("/department")
		{
			...
			department.Get("/deleted", config.DepartmentHandler.GetAllDeletedDepartment)
			...
			department.Post("/restore/:id", config.DepartmentHandler.RestoreDepartment)
			department.Get("/:id", config.DepartmentHandler.GetDepartmentByID)
		}

		patient := admin.Group("/patient")
		{
			patient.Get("/", config.PatientHandler.GetAllPatient)
			patient.Get("/deleted", config.PatientHandler.GetAllDeletePatient)
			patient.Post("/create", config.PatientHandler.CreatePatient)
			patient.Get("/:id", config.PatientHandler.GetByIDPatient)
		...
		testType := admin.Group("/test-type")
		{
			testType.Get("/", config.TestTypeHandler.GetAllTestType)
			testType.Get("/deleted", config.TestTypeHandler.GetAllDeleteTestType)
			testType.Post("/create", config.TestTypeHandler.CreateTestType)
			testType.Get("/:id", config.TestTypeHandler.GetTestTypeByID)
			testType.Put("/update/:id", config.TestTypeHandler.UpdateTestType)
			testType.Delete("/:id", config.TestTypeHandler.DeleteTestType)
			testType.Delete("/permanent/:id", config.TestTypeHandler.HardDeleteTestType)
			testType.Post("/restore/:id", config.TestTypeHandler.RestoreTestType)
		}
